...
class SQLiteMineco:
    ...
    def _crear_o_actualizar_tabla(self):
        ...
        if 'variacion_mensual' not in [col[1] for col in self.cursor.fetchall()]:
            self.cursor.execute(
                "ALTER TABLE indicadores_economicos ADD COLUMN variacion_mensual REAL")
        self.conn.commit()
    ...
    def mostrar_datos(self, limit=10):
        try:
            ...
            for row in rows:
                ...
                print(
                    f"Fecha: {row[0]} | Valor: {row[1]:.2f} | Indicador: {row[2]} | Var. Mensual: {var_mensual} | Insertado: {row[4]}")
            print("-" * 80)
        except Exception as e:
